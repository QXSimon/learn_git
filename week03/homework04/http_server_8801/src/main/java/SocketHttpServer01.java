 ...
 */
public class SocketHttpServer01 {
    public static void main(String[] args) throws IOException {
        ExecutorService executorService = Executors.newFixedThreadPool(2);
        final ServerSocket serverSocket = new ServerSocket(8801);
        final SocketHttpServer01 server01 = new SocketHttpServer01();
        while(true){
            try{
                final Socket socket=serverSocket.accept();
                executorService.execute(()-> server01.service(socket) );
            }catch (IOException e){
                throw e;
            }
        }
    ...
    private void service(Socket socket)  {
        try(PrintWriter printWriter = new PrintWriter(socket.getOutputStream(),true); socket){
            System.out.println("8801 received remote request: "+socket.getRemoteSocketAddress().toString());
